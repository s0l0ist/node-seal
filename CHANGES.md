...
**node-seal v7.0.0** is a full rewrite. The custom JS wrapper layer is gone -
the package now exposes the Emscripten-generated bindings directly. This makes
the package much smaller and faster, but it also means most consumers will have
to update their code.

### Highlights

- **📦 Package size**: ~70 MB => **~2.8 MB** (~96% smaller)
  - WASM is no longer base64-inlined.
  - No more bundled/transpiled wrapper code.
  - Better CDN/browser compression.
- **⚡ Performance**:
  - Calls go straight to WASM (no wrapper indirection).
  - Data marshaling is reduced to a single copy where possible.
  - Native ESM => better tree-shaking and module resolution.
...
The old deep-import layout like
`node-seal/<throws|allows>_wasm_<node|web|worker>_<umd|es>` is no longer needed
in v7.
...
// Default (throws)
import SEAL from 'node-seal'
// Explicit variants, but no environment-specific bundle names
...
Saving is now explicit about compression - you **must** pass a compression mode.

```ts
// Before (v6)
// Compression was optional (defaulted to zstd)
const base64: string = ciphertext.save()
const array: Uint8Array = ciphertext.saveArray(seal.ComprModeType.none)
...
Batch encoding now expects 64-bit integer typed arrays; BigInt is handled
natively by Emscripten.

```ts
// Before (v6)
batchEncoder.encode(Int32Array.from([1, 2, 3]))
batchEncoder.encode(Uint32Array.from([1, 2, 3]))
batchEncoder.encode(BigInt64Array.from([1n, 2n, 3n]))
batchEncoder.encode(BigUint64Array.from([1n, 2n, 3n]))

// After (v7)
// 32-bit typed arrays removed
batchEncoder.encode(BigInt64Array.from([1n, 2n, 3n]))
batchEncoder.encode(BigUint64Array.from([1n, 2n, 3n]))
```
...
- ESM-first: `"type": "module"` in `package.json`; direct Emscripten ESM export;
  no bundling; works with Vite/Webpack/Rollup/esbuild.

- TypeScript: autogenerated via `--emit-tsd`, always in sync with C++ bindings;
  handwritten TS defs removed.

- WASM distribution: shipped as a separate `.wasm `file => works with
  `DYNAMIC_EXECUTION=false` (e.g. Cloudflare Workers) and allows better CDN
  caching.

- Exceptions: native WebAssembly exceptions enabled; legacy handling removed;
  errors now throw normally.
...
- Testing: Jest => Vitest (faster, better ESM/WASM).
- Build: Rollup removed; build is "TS compile + copy WASM artifacts"; no custom
  transforms.
...
Even with the breaking changes, **v7.0.0** gives you:

- ✅ ~96% smaller package
- ✅ Faster runtime (no wrapper layer)
- ✅ Better TS support (auto-generated, in-sync types)
- ✅ Explicit, predictable memory management
- ✅ Direct mapping to the underlying C++ API
- ✅ Modern ESM-first packaging

For detailed API semantics, **see the Microsoft SEAL C++ documentation**.
...
- Many internal enhancements to clean up the C++ bindings focused on reducing
  memory.
  - Replaced `ostringstream`/`istringstream` paths with direct buffer
    `save()`/`load()` overloads.
